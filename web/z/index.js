...
// 登录注册显示和消失弹出框   这是模态框的显示和关闭   以后模态框都可以这样写
...
$("#regEmail").blur(function(){
    ...
    //alert(emailVal);
    // js 判断不相等  不能使用   !""==xxxx    注意！！！！！！！！！！！！！！！！！！！！！
    if(null != emailVal && ""!=emailVal){
        var params={"userEmail":emailVal};
       // alert(params);
        $.post("UserController/validateEmail.action",params,function(data){
            if(data=="success"){
...
function commitLogin(){
   ...
   if(null!=email && email!="" && null!=password && password!=""){
       ...
       // alert(params);
        // post要小写
        $.post("UserController/userLogin.do",params,function(data){
                 if(data=='success'){
                      document.location.reload();
                   }
        });
